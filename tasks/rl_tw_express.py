...
utilities.install('textworld-express')

from textworld_express import TextWorldExpressEnv

tw_exp_path = f"{APP_ROOT}/cache/tw_exp_data"
os.makedirs(tw_exp_path, exist_ok=True)
...
def extract_location(infos):
    ...
    result = re.search(r"-= (.*?) =-", infos["observation"])
    location = None
...
class Textworld_Transition(MDP_Transition):
    ...
    def eval(self, node, obs):
        ...
        if self == progress_transition:
            ...
            if infos["tasksuccess"] or infos["taskfailure"]:
                terminated = True
                truncated = False
                succeeded = False
                mdp_score = mdp_score - 5
...
def play(env, env_step, persona, rollout_file_path, nb_episodes=10, verbose=False, verbose_step=10, verbose_prefix=""):
    ...
    for no_episode in range(1, nb_episodes + 1):
        obs, infos = env.reset(seed=no_episode, gameFold="train")
        obs = infos["observation"]
        obs = re.sub(r"\n+", "\n", obs)
        score = 0
        done = False

        objective_transition = Textworld_Main_Goal(infos["taskDescription"], 1)
...
if __name__ == "__main__":
    ...
    experiment_path = f"{APP_ROOT}/experiments/rl_tw_express"
    if args.reset:
    ...
    play(env, env_step, persona, 
        rollout_file_path=rollout_file_path, 
        nb_episodes=10000, verbose=True, verbose_step=100)
    # persona.save(agent_parameter_path)

    persona.set_training_mode(False)
    play(env, env_step, persona, 
        rollout_file_path=rollout_file_path, 
        nb_episodes=10, verbose=True, verbose_step=1)
